...
const AUTHORS_FILE     = path.join(DATA, "authors.json");
...
app.post("/api/posts", auth, function(req, res) {
  ...
  var p = { id: Date.now(), title: b.title, author: b.author || "Redacção Vértice",
...
app.put("/api/posts/:id", auth, function(req, res) {
  ...
  if (i === -1) return res.status(404).json({ error: "Post não encontrado." });
...
app.delete("/api/posts/:id", auth, function(req, res) {
  var posts = readJSON(POSTS_FILE).filter(function(p) { return p.id !== Number(req.params.id); });
  writeJSON(POSTS_FILE, posts); res.json({ success: true });
});
...
app.put("/api/books/:id", auth, function(req, res) {
  ...
  if (i === -1) return res.status(404).json({ error: "Livro não encontrado." });
...
app.delete("/api/books/:id", auth, function(req, res) {
  var books = readJSON(BOOKS_FILE).filter(function(b) { return b.id !== Number(req.params.id); });
  writeJSON(BOOKS_FILE, books); res.json({ success: true });
});

// ── Authors ───────────────────────────────────────────────
app.get("/api/authors", function(_q, res) { res.json(readJSON(AUTHORS_FILE)); });
app.post("/api/authors", auth, function(req, res) {
  var b = req.body;
  if (!b.nome) return res.status(400).json({ error: "Nome obrigatório." });
  var authors = readJSON(AUTHORS_FILE);
  var a = { id: Date.now(), nome: b.nome, foto: b.foto || "", bio: b.bio || "", obras: b.obras || [], genero: b.genero || "" };
  authors.push(a); writeJSON(AUTHORS_FILE, authors); res.status(201).json(a);
});
app.put("/api/authors/:id", auth, function(req, res) {
  var authors = readJSON(AUTHORS_FILE);
  var i = authors.findIndex(function(a) { return a.id === Number(req.params.id); });
  if (i === -1) return res.status(404).json({ error: "Autor não encontrado." });
  authors[i] = Object.assign({}, authors[i], req.body); writeJSON(AUTHORS_FILE, authors); res.json(authors[i]);
});
app.delete("/api/authors/:id", auth, function(req, res) {
  var authors = readJSON(AUTHORS_FILE).filter(function(a) { return a.id !== Number(req.params.id); });
  writeJSON(AUTHORS_FILE, authors); res.json({ success: true });
});
